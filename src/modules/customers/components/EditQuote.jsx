...
export const EditQuote = ({
  ...
  const { data: listMembership = [] } = useListMembershipTypeQuery({
    bundle_type_id: category,
  })
        ...
        onSubmit={(values) => {
          ...
          products.map((product) => {
            if (product.currencies?.unit_amount !== 0) {
              values.totalSetup =
                values.totalSetup - product.currencies.setup_fee
            }
            return product
          })
          ...
          <>
            <Form className={form} layout="vertical" autoComplete="off">
              <Form.Item
                label="Project Name"
                validateStatus={
                  errors.project_name && touched.project_name && "error"
                }
              ...
              <Form.Item
                ...
                required
                validateStatus={
                  errors.bundle_type_id && touched.bundle_type_id && "error"
                }
              ...
              >
                <Select
                  disabled={
                    programs?.find(({ value }) => value === values.program)
                      ?.has_idx
                      ? false
                      : true
                  }
            ...
            >
              <Checkbox
                onChange={(e) =>
                  setFieldValue("send_email", e.target.checked ? 1 : 0)
                }
